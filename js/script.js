...
serviceCards.forEach(card => {
    ...
    if (link) {
        link.addEventListener('click', (e) => {
            ...
            showNotification(`Loading ${serviceName}...`, 'success');
        });
    }
});
...
ctaButtons.forEach(button => {
    button.addEventListener('click', () => {
        if (button.textContent.includes('Provider')) {
            showNotification('Redirecting to provider registration...', 'success');
        } else {
            const servicesSection = document.querySelector('#services');
            if (servicesSection) {
                servicesSection.scrollIntoView({ behavior: 'smooth' });
...
document.querySelectorAll('.service-card, .provider-card, .step, .feature, .testimonial-card').forEach(el => {
    el.style.opacity = '0';
